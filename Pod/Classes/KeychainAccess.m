...
+ (NSString *)uid {
    return [[[KeychainAccess alloc] initWithKey:@"deviceUID"] uid];
}
 ...
 */
- (void)save {
    if (![KeychainAccess valueForUserDefaultsKey:_uidKey]) {
        [KeychainAccess setValue:self.uid forUserDefaultsKey:_uidKey];
    }
    if (![KeychainAccess valueForKeychainKey:_uidKey service:_uidKey]) {
        [KeychainAccess setValue:self.uid forKeychainKey:_uidKey inService:_uidKey];
    }
}
